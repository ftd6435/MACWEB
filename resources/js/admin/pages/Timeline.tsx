    ...
    return (
        <div className="space-y-10 pb-20">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-[#212121] tracking-tight">Historique de l'Entreprise</h1>
            ...
            <AnimatePresence mode="wait">
                {isLoading ? (
                    <div className="space-y-6">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="bg-white h-32 rounded-[2.5rem] border border-[#F1F5F9] animate-pulse" />
                        ))}
                    </div>
                ) : (
                                ...
                                >
                                    ...
                                    <div className="flex-1 bg-white border border-[#F1F5F9] rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl hover:-translate-y-1 smooth-animation flex flex-col md:flex-row items-center gap-8">
                                        <div className="shrink-0 text-center md:text-left">
                                            <span className="text-3xl font-black text-[#00B8D4]">{milestone.year}</span>
                                        </div>
                                        
                                        <div className="flex-1 text-center md:text-left">
                                            <h3 className="text-xl font-black text-[#212121] mb-2">{milestone.title}</h3>
                                            <p className="text-sm text-[#757575] font-medium leading-relaxed">{milestone.description}</p>
                                        </div>

                                        <div className="flex items-center space-x-3 shrink-0">
                                            <div className="flex flex-col space-y-1 mr-2">
                                                <button className="p-1.5 hover:bg-[#F8FAFC] rounded-lg text-[#9E9E9E] hover:text-[#00B8D4] smooth-animation"><ChevronUp className="w-4 h-4" /></button>
                                                <button className="p-1.5 hover:bg-[#F8FAFC] rounded-lg text-[#9E9E9E] hover:text-[#00B8D4] smooth-animation"><ChevronDown className="w-4 h-4" /></button>
                                            </div>
                                            <button 
                    ...
                    <>
                        ...
                        >
                            <div className="p-10 flex items-center justify-between border-b border-[#F1F5F9] shrink-0">
                                <div>
                                    <h2 className="text-2xl font-black text-[#212121]">{editingMilestone ? "Modifier l'Événement" : "Nouvel Événement"}</h2>
                                    <p className="text-xs text-[#9E9E9E] font-bold uppercase tracking-widest mt-1">Étape marquante de l'entreprise</p>
                                </div>
                            ...
                            <form onSubmit={handleSave} className="p-10 space-y-8 overflow-y-auto custom-scrollbar">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    ...
                                    <div className="md:col-span-2 space-y-2">
                                        <label className="text-xs font-black text-[#212121] uppercase tracking-widest ml-1 flex items-center">
                                            <Type className="w-3.5 h-3.5 mr-2 text-[#00B8D4]" /> Titre de l'Étape
                                        </label>
                                        <input 
                                            type="text" 
                                            required
