    ...
    return (
        <div className="space-y-10 pb-20">
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-[#212121] tracking-tight">Gestion de l'Équipe</h1>
            ...
            <AnimatePresence mode="wait">
                {isLoading ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {[1, 2, 3, 4].map(i => (
                            <div key={i} className="bg-white h-80 rounded-[2.5rem] border border-[#F1F5F9] animate-pulse" />
                        ))}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                        {filteredTeam.map((member, idx) => (
                            ...
                            >
                                <div className="relative mb-6">
                                    ...
                                    <div className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-xl border-4 border-white flex items-center justify-center shadow-md ${member.is_active ? "bg-emerald-500" : "bg-red-500"}`}>
                                        {member.is_active ? <CheckCircle className="w-4 h-4 text-white" /> : <XCircle className="w-4 h-4 text-white" />}
                                    </div>
                                </div>

                                <h3 className="text-lg font-black text-[#212121] mb-1 group-hover:text-[#00B8D4] smooth-animation">{member.name}</h3>
                                <p className="text-[10px] font-black text-[#00B8D4] uppercase tracking-[0.2em] mb-4">{member.role}</p>
                                ...
                                <div className="flex items-center space-x-2 mt-auto">
                                    ...
                                    {member.linkedin && (
                                        <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="p-3 bg-[#F8FAFC] text-[#616161] hover:bg-blue-50 hover:text-blue-600 rounded-xl smooth-animation">
                    ...
                    <>
                        ...
                        >
                            <div className="p-10 flex items-center justify-between border-b border-[#F1F5F9] shrink-0">
                                <div>
                                    <h2 className="text-2xl font-black text-[#212121]">{editingMember ? "Modifier le Membre" : "Ajouter un Membre"}</h2>
                                    <p className="text-xs text-[#9E9E9E] font-bold uppercase tracking-widest mt-1">Informations du personnel</p>
                                </div>
                            ...
                            <form onSubmit={handleSave} className="p-10 space-y-8 overflow-y-auto custom-scrollbar">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        <label className="text-xs font-black text-[#212121] uppercase tracking-widest ml-1 flex items-center">
                                            <User className="w-3.5 h-3.5 mr-2 text-[#00B8D4]" /> Nom Complet
                                        </label>
                                        <input 
                                            type="text" 
                                            required
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black text-[#212121] uppercase tracking-widest ml-1 flex items-center">
                                            <Briefcase className="w-3.5 h-3.5 mr-2 text-[#00B8D4]" /> Poste / Rôle
                                        </label>
                                        <input 
                                            type="text" 
                                            required
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-black text-[#212121] uppercase tracking-widest ml-1 flex items-center">
                                        <LinkIcon className="w-3.5 h-3.5 mr-2 text-[#00B8D4]" /> Profil LinkedIn
                                    </label>
                                    <input 
                                        type="url" 
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-black text-[#212121] uppercase tracking-widest ml-1 flex items-center">
                                        <Type className="w-3.5 h-3.5 mr-2 text-[#00B8D4]" /> Biographie Courte
                                    </label>
                                    <textarea 
                                        rows={3}
                                        value={formData.bio || ""}
                                ...
                                <div className="flex items-center justify-between p-6 bg-[#F8FAFC] rounded-3xl">
                                    <div className="flex items-center space-x-3">
                                        ...
                                        <span className="text-xs font-black text-[#212121] uppercase tracking-widest">Activer le membre</span>
                                    </div>
                                </div>
